...
volatile uint32_t sendOk = 0, sendFail = 0, beaconCount = 0, deauthCount = 0;
...
void setup() {
  ...
  delay(100);
  Serial.println();
  Serial.println("=== DEAUTH + BEACON SPAM v3 ===");
  Serial.println("Commands: /status /help");
...
void loop() {
  ...
  if (now - lastReport >= 3000) {
    lastReport = now;
    Serial.printf("[STAT] ok=%lu fail=%lu beacon=%lu deauth=%lu ch=%u ap=%d/%d uptime=%lus\n",
                  (unsigned long)sendOk, (unsigned long)sendFail, (unsigned long)beaconCount,
                  (unsigned long)deauthCount, wifi_get_channel(), networkCount, networkCount,
    ...
    if (c == '\n') {
      ...
      if (input.length() > 0) {
        if (input.startsWith("/status")) {
          ...
          Serial.printf("[STATUS] APs=%d ch=%d beacon=%lu deauth=%lu ok=%lu fail=%lu uptime=%lus\n",
                        networkCount, wifi_get_channel(), (unsigned long)beaconCount,
                        (unsigned long)deauthCount, (unsigned long)sendOk, (unsigned long)sendFail, elapsed);
